// /api/staff.js
// Handles ALL staff + leave routes via ?resource= and ?action= params
//
// ── STAFF routes ──────────────────────────────────────────────────────────────
...
//   POST   ?resource=leave&action=submit   → staff submits a leave request
//   POST   ?resource=leave&action=respond  → manager approves / denies a request
//
// Collection: staff, staff_day_status, leave_requests
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // STAFF ROUTES  (original — unchanged)
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    if (req.method === 'GET') {
      ...
      // If branch provided → filter by branch (for job board / staff portal)
      // If no branch → return ALL active staff (for admin staff directory)
      ...
      // Day statuses only relevant when branch + date are given
      const statusMap = {};
      if (branch) {
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'login') {
        ...
        const member = await staffCol.findOne({
    ...
    if (req.method === 'PATCH') {
      ...
      if (a === 'clock_in') {
        await dayCol.updateOne(filter,
          { $set: { status: 'active', clockInAt: now }, $setOnInsert: { bayNumber: null, breakLogs: [] } },
          opts
        );
      } else if (a === 'clock_out') {
        ...
        await dayCol.updateOne(filter,
          { $set: { status: 'on_break' }, $push: { breakLogs: { startedAt: now, endedAt: null } } },
          opts
        );
      } else if (a === 'end_break') {
